...
def spike_calibration(isodata, spike_measurement, mixture_measurement, isoinv = None, standard=None):
    ...
    VAms = [calcratiocov(mixture_measurement[i,:],emod_mixture,deno) for i in range(n_m)]
    ...
    res = minimize(objective, z0, args=(m, t, P, n, Sms, Sts, n_m, n_t), jac =True, tol = 1e-16, options={'disp':False ,'gtol':1e-8,'eps':1e-12})
    z = res.x
    misfit = res.fun
    ...
    if len(unused)>0:
        ...
        expected_spike = expected_spike_measurement*np.exp(-np.log(isodata.mass)*expected_betat)
...
def dmt_expected_dz(z, P, n, n_m, n_t):
    ...
    for i in range(n_m):
        ...
        dmdz[i,:,2*n_m+n_t:] = dindividual_m_expected_dT(lambda_[i], beta[i], T, P, n)
...
if __name__=="__main__":
    ...
    print("Difference between true spike and calibrated:", out['calibrated_spike'] - true_spike)
